  ...
  const handleLogin = async () => {
    ...
    try {
      const response = await fetch("http://172.20.10.5/cuahangtaphoa/Login/Login", {
        ...
        body: `TenDangNhap=${encodeURIComponent(username.trim())}&MatKhau=${encodeURIComponent(password.trim())}`, // ← đổi body
      });
...
const styles = StyleSheet.create({
  ...
  scrollContent: {
    ...
    alignItems: "center",
    paddingVertical: 50,        // Tăng khoảng cách để scroll thoải mái hơn
  },
  circleTopRight: {
    position: "absolute",
    width: 200,
